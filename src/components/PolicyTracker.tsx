...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  return (
    <Card className={`relative overflow-hidden border-2 ${borderClass} hover:shadow-lg transition-all duration-300 ${cardBgClass}`}>
      <CardContent className="p-3 md:p-4">
        <div className="flex items-start justify-between mb-2 md:mb-3">
          <div className="flex items-center gap-2">
            <div className={`w-9 h-9 md:w-10 md:h-10 ${iconBgClass} rounded-lg flex items-center justify-center shadow flex-shrink-0`}>
              {icon}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-gray-800 uppercase tracking-wide mb-0.5 break-words">{title}</p>
              {trend && (
                <div className="flex items-center gap-1.5">
                  {trend === 'up' ? (
                    <TrendingUp className={`w-3 h-3 ${textClass} flex-shrink-0`} />
                  ) : (
                    <TrendingDown className={`w-3 h-3 ${textClass} flex-shrink-0`} />
                  )}
                  <span className="text-[10px] text-gray-600 font-medium">vs last quarter</span>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-2 md:gap-3 mb-2 md:mb-3">
          <div className="bg-white/60 rounded-lg p-2 md:p-3 border border-gray-200">
            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-0.5">{t('tracker.current')}</p>
            <p className={`text-base md:text-xl font-bold ${textClass} leading-tight break-words`}>{current}</p>
          </div>
          <div className="bg-white/60 rounded-lg p-2 md:p-3 border border-gray-200">
            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-0.5">{t('tracker.target')}</p>
            <p className={`text-base md:text-xl font-bold ${textClass} leading-tight break-words`}>{target}</p>
          </div>
        </div>
        ...
        {solution && (
          <div className="mt-3 pt-3 border-t border-gray-200">
            <button
              ...
              className="w-full flex items-center justify-between text-left group hover:bg-white/50 rounded-md p-2 -m-2 transition-colors"
            >
              <span className={`text-sm font-bold ${textClass}`}>{t('tracker.fix')}</span>
              {isExpanded ? (
                <ChevronUp className={`w-4 h-4 ${textClass} group-hover:scale-110 transition-transform`} />
              ) : (
                <ChevronDown className={`w-4 h-4 ${textClass} group-hover:scale-110 transition-transform`} />
              )}
            </button>
            {isExpanded && (
              <div className="mt-2 text-xs text-gray-700 leading-relaxed animate-in slide-in-from-top-2 duration-200 bg-white/40 rounded-md p-2 md:p-3">
                {solution}
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl shadow-xl p-4 md:p-5 border border-gray-100 transform -translate-y-12">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <MetricCard
              ...
              current="28.92%"
              target="12%"
              icon={<TrendingUp className="w-5 h-5 text-[#ef8636]" />}
              color="[#ef8636]"
              trend="down"
            ...
            <MetricCard
              ...
              current="4.7%"
              target="2.8%"
              icon={<Users className="w-5 h-5 text-[#1daa62]" />}
              color="[#1daa62]"
              trend="down"
            ...
            <MetricCard
              ...
              current="2.7%"
              target="6.0%"
              icon={<BarChart3 className="w-5 h-5 text-[#ef8636]" />}
              color="[#ef8636]"
              trend="up"
            ...
            <MetricCard
              ...
              current="₦1,350"
              target="₦650"
              icon={<DollarSign className="w-5 h-5 text-[#1daa62]" />}
              color="[#1daa62]"
              trend="down"
